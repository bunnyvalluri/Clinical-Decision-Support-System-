...
import { Activity, HeartPulse, RefreshCw, Search, Users } from "lucide-react";
...
export default function DoctorSearchPage() {
  ...
  const [activeTab, setActiveTab] = useState<"patients" | "predictions" | "clinical_records">("patients");
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold text-slate-600">Assigned Cohort & Risk Search</span>
              </div>
              <h1 className="mt-2 text-2xl font-bold tracking-tight text-slate-900">
                Doctor Clinical Search Hub
              </h1>
              <p className="mt-1 text-xs text-slate-500">
                Targeted search across assigned patients, deterioration risk scores, and encounter timelines.
              </p>
            </div>
            <button
          ...
          <div className="mt-6 flex items-center gap-2 border-b border-slate-100 pb-3">
            {[
              ...
              { id: "predictions", label: "Risk Assessments", icon: HeartPulse },
              { id: "clinical_records", label: "Encounter Records", icon: Activity },
            ].map((tab) => {
              const Icon = tab.icon;

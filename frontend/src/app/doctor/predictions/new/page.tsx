...
function DoctorNewPredictionContent() {
  ...
  const handleRunInference = async (e: React.FormEvent) => {
    ...
    const newPredictionRecord = {
      id: predictionId,
      patient_id: selectedPatient?.id || "p-101",
      patient_mrn: selectedPatient?.mrn || "MRN-90241",
      patient_name: selectedPatient ? `${selectedPatient.first_name} ${selectedPatient.last_name}` : "Eleanor Ward",
      timestamp: `${now.toISOString().substring(0, 10)} ${predictionTime}`,
      risk_level: riskLevel,
      probability: Math.round(finalProb * 1000) / 1000,
      confidence_interval: [
        Math.max(0, Math.round((finalProb - 0.05) * 100) / 100),
        Math.min(1, Math.round((finalProb + 0.05) * 100) / 100),
      ] as [number, number],
      ...
      clinical_factors: {
        ...
        exercise_angina: isExAngina ? "Yes" : "No",
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5 p-4 sm:p-6 lg:p-8">
      ...
      <div className="p-3.5 rounded-xl border border-blue-200 bg-blue-50/70 text-xs text-blue-900 flex items-start gap-2.5">
      ...
      <form onSubmit={handleRunInference}>
        <div className="rounded-xl border border-slate-200 bg-white shadow-xs overflow-hidden">
          ...
          <div className="p-5 sm:p-6 space-y-6">
            ...
            {/* Row 2: Resting BP, Cholesterol, Fasting BS */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <Input
            ...
            {/* Row 3: Resting ECG, Max Heart Rate, Exercise Angina */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <Select

        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {ROLE_FEATURE_CARDS.map((card) => {
            ...
            return (
              <div
                key={card.role}
                className="rounded-2xl bg-slate-50/70 border border-slate-200 p-6 sm:p-7 flex flex-col justify-between shadow-2xs hover:border-slate-300 hover:shadow-xs transition-all duration-200"
              >
                <div className="space-y-5">
                  <div className="flex items-center justify-between">
                    <div className="h-12 w-12 rounded-2xl bg-white border border-slate-200/80 text-teal-700 flex items-center justify-center shadow-xs">
                ...
                <div className="pt-6 mt-6 border-t border-slate-200 flex items-center justify-between">
                  <Link href={card.route}>
                    <Button
                      variant="outline"
                      size="sm"
                      className="text-xs font-semibold border-slate-300 hover:bg-white gap-1.5"
                    >

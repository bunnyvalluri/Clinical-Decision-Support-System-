...
export function RoleFeatures() {
  return (
    <section id="role-features" className="py-20 sm:py-28 bg-white border-b border-slate-200/80 relative">
          ...
          {ROLE_FEATURE_CARDS.map((card) => {
            ...
            return (
              <div
                key={card.role}
                tabIndex={0}
                className="group rounded-2xl bg-slate-50/70 border border-slate-200/90 p-6 sm:p-7 flex flex-col justify-between shadow-xs hover:border-teal-400 hover:shadow-xl hover:shadow-teal-500/5 hover:-translate-y-1.5 transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500"
              >
                <div className="space-y-5">
                  <div className="flex items-center justify-between">
                    <div className="h-12 w-12 rounded-2xl bg-white border border-slate-200 text-teal-700 flex items-center justify-center shadow-xs group-hover:scale-110 transition-transform duration-300">
                      <IconComponent className="h-6 w-6" />
                    </div>
                    <span className="text-[10px] font-mono font-bold px-2.5 py-1 rounded-md bg-white text-teal-800 border border-teal-200 uppercase tracking-wide shadow-2xs">
                      {card.badge}
                    </span>
                  </div>
                ...
                <div className="pt-6 mt-6 border-t border-slate-200/80 flex items-center justify-between">
                  <Link href={card.route}>
                    <Button
                      variant="outline"
                      size="sm"
                      className="text-xs font-bold border-slate-300 bg-white hover:bg-slate-50 text-slate-800 hover:text-slate-950 gap-1.5 rounded-xl shadow-2xs transition-all hover:-translate-y-0.5"
                    >
